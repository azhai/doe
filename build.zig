...
pub fn build(b: *std.Build) !void {
    ...
    if (target.os.tag == .wasi) {
        ...
        abs_root_path = try std.Io.Dir.cwd().realPathFileAlloc(b.graph.io, ".", b.allocator);
    }
    ...
    {
        const step = b.step("cli", "Build CLI tools (doer and doec).");

        // Helper function to create a CLI executable with common settings
        const createCliExe = struct {
            fn create(builder: *std.Build, mod: *std.Build.Module, name: []const u8, strip_: bool) *std.Build.Step.Compile {
                const exe = builder.addExecutable(.{
                    .name = name,
                    .root_module = mod,
                });
                exe.root_module.strip = strip_;
                exe.root_module.addIncludePath(builder.path("src"));

                // Allow dynamic libraries to be loaded by filename in the cwd.
                if (target.os.tag == .linux) {
                    exe.root_module.addRPathSpecial(":$ORIGIN");
                    if (target.cpu.arch == .x86_64) {
                        exe.root_module.addRPathSpecial("/usr/lib/x86_64-linux-gnu");
                    }
        ...
        // Build doer (VM backend, REPL)
        const doer_exe = createCliExe(b, cli_mod, "doer", strip);
        step.dependOn(&b.addInstallArtifact(doer_exe, .{}).step);

        // Build doec (JIT compiler)
        const doec_exe = createCliExe(b, cli_mod, "doec", strip);
        step.dependOn(&b.addInstallArtifact(doec_exe, .{}).step);

        // Install builtin modules for both executables
        if (optimize != .Debug) {
            step.dependOn(&b.addInstallFile(b.path("src/builtins/core.do"), "bin/src/builtins/core.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/builtins/cy.do"), "bin/src/builtins/cy.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/builtins/meta.do"), "bin/src/builtins/meta.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/builtins/c.do"), "bin/src/builtins/c.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/math.do"), "bin/src/std/math.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/os.do"), "bin/src/std/os.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/io.do"), "bin/src/std/io.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/cli.do"), "bin/src/std/cli.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/test.do"), "bin/src/std/test.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/os.macos.do"), "bin/src/std/os.macos.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/os.linux.do"), "bin/src/std/os.linux.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/os.windows.do"), "bin/src/std/os.windows.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/windows/windows.do"), "bin/src/std/windows/windows.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/windows/kernel32.do"), "bin/src/std/windows/kernel32.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/libc.do"), "bin/src/std/libc.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/libc.macos.do"), "bin/src/std/libc.macos.do").step);
            step.dependOn(&b.addInstallFile(b.path("src/std/libc.linux.do"), "bin/src/std/libc.linux.do").step);
        }
    }
...
fn run_lib_standalone_test(b: *std.Build, path: []const u8, lib_bin: std.Build.LazyPath) *std.Build.Step.Run {
    ...
    main.root_module.addObjectFile(lib_bin);
...
fn create_lib_config(b: *std.Build, opts: Options) !*std.Build.Module {
    const buildTag = b.graph.environ_map.get("BUILD") orelse "0";
    const commitTag = b.graph.environ_map.get("COMMIT") orelse "local";
    const options = b.addOptions();
    options.addOption(bool, "dev", dev);
    if (dev) {
        options.addOption([]const u8, "version", b.fmt("{s}-DEV", .{Version}));
        options.addOption([]const u8, "full_version", b.fmt("Doe {s}-DEV build-{s}-{s}", .{ Version, buildTag, commitTag }));
    } else {
        options.addOption([]const u8, "version", Version);
        options.addOption([]const u8, "full_version", b.fmt("Doe {s} build-{s}-{s}", .{ Version, buildTag, commitTag }));
    }
...
pub const PrintStep = struct {
    ...
    fn make(step: *std.Build.Step, _: std.Build.Step.MakeOptions) anyerror!void {
        ...
        std.Io.File.stdout().writeStreamingAll(step.owner.graph.io, self.str) catch unreachable;
    }
};
...
pub fn buildCVM(b: *std.Build, opts: Options) !*std.Build.Step.Compile {
    ...
    lib.root_module.addCSourceFile(.{ .file = b.path("src/vm.c"), .flags = cflags.items });
    for (isystem) |path| {
