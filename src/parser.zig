...
pub const Parser = struct {
    ...
    fn parseOptName(self: *Parser) !?*ast.Node {
        ...
        var token = self.peek();
        switch (token.tag()) {
            .with_k, .scope_k, .void_k, .struct_k, .global_k, .cstruct_k, .cunion_k, .trait_k, .enum_k, .type_k, .error_k, .none_k, .ident => {
                self.advance();
    ...
    fn parseTypeDecl(self: *Parser, start: u32, config: TypeDeclConfig) !*ast.Node {
        ...
        switch (self.peek().tag()) {
            ...
            // `struct` is optional.
            .left_paren, .struct_k, .new_line, .colon => {
    ...
    fn parseOptTypeSpec2(self: *Parser) anyerror!?*ast.Node {
        ...
        switch (token.tag()) {
            // `@extern fn()`
            .at, .left_paren, .space_left_bracket, .left_bracket, .star, .question, .ampersand, .bang, .caret, .pound, .void_k, .type_k, .error_k, .percent, .switch_k, .ident => {
    ...
    fn newCUnionDecl(self: *Parser, start: TokenId, name: *ast.Node, config: TypeDeclConfig, impls: []*ast.ImplDecl, cases: []*ast.UnionCase) !*ast.CUnionDecl {
    ...
    fn newStructDecl(self: *Parser, start: TokenId, node_t: ast.NodeType, name: *ast.Node, config: TypeDeclConfig, impls: []*ast.ImplDecl, fields: []*ast.Field, num_embedded_fields: usize, is_tuple: bool) !*ast.StructDecl {
    ...
    fn parseTightTermWithLeft(self: *Parser, left_: *ast.Node, config: ParseTermConfig) !*ast.Node {
        ...
        while (true) {
            ...
            switch (next.tag()) {
                ...
                .dot_dot_rangle, .dot_dot_rangle_equal, .dot2, .right_bracket, .right_paren, .right_brace, .else_k, .comma, .colon, .equal, .plus, .minus, .star, .slash, .ampersand, .vert_bar, .double_ampersand, .double_vert_bar, .double_left_angle, .double_right_angle, .double_star, .caret, .left_angle, .left_angle_equal, .right_angle, .right_angle_equal, .percent, .equal_equal, .bang_equal, .and_k, .or_k, .minus_right_angle, .bin, .oct, .hex, .dec, .float, .if_k, .stringt, .stringt_multi, .equal_right_angle, .new_line, .null => break,
                else => break,
            }
        }
        return left;
    ...
    fn parseTightTermLeft(self: *Parser, config: ParseTermConfig) !?*ast.Node {
        ...
        switch (token.tag()) {
            ...
            .space_left_bracket, .left_bracket => {
                self.advance();
    ...
    fn parseExprWithLeft(self: *Parser, start: u32, left_: *ast.Node, config: ParseExprConfig) !*ast.Node {
        ...
        while (true) {
            ...
            switch (next.tag()) {
                ...
                .double_vert_bar, .double_ampersand, .double_left_angle, .double_right_angle, .tilde, .plus, .minus, .star, .slash => {
                ...
                .double_star, .left_angle, .left_angle_equal, .right_angle, .percent, .equal_equal, .bang_equal, .and_k, .or_k, .right_angle_equal => {
                    const bin_op = toBinExprOp(next.tag()).?;
                    const op_start = self.next_pos;
                    self.advance();
                    const right = try self.parseRightExpr(bin_op);
    ...
    fn parseExprOrAssignStatement(self: *Parser, allow_block: bool) !?*ast.Node {
        ...
        switch (tag) {
            ...
            .double_left_angle, .double_right_angle, .double_vert_bar, .double_ampersand, .plus, .tilde, .minus, .star, .slash => {
                // +=, -=, etc.
                self.advance();
                self.advance();
                is_assign_stmt = true;
        ...
        switch (expr.type()) {
            .callExpr, .accessExpr, .deref, .index_expr, .ident => {},
            else => {
...
fn isRecordKeyNodeType(node_t: ast.NodeType) bool {
    switch (node_t) {
        .ident, .sq_string_lit, .decLit, .binLit, .octLit, .hexLit => {
            return true;
        },
        else => {
            return false;
