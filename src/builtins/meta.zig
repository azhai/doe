...
pub fn dump_frame(t: *cy.Thread) !C.Ret {
    ...
    const prev_pc = t.c.fp[1].getRetPcPtr();
    const prev_fp = t.c.fp[2].getRetFramePtr();
    ...
    try w.print("info={}, ret_pc={*}, ret_fp={*}\n", .{
        ...
        frame[func_info.ret_size + 1].getRetPcPtr(),
        frame[func_info.ret_size + 2].getRetFramePtr(),
    });
...
pub fn load_(c: *cy.Chunk, cx: *cy.ConstEvalContext) !cy.TypeValue {
    ...
    const full_path = try std.fmt.allocPrint(c.alloc, "{s}/{s}", .{ root_path, path });
    defer c.alloc.free(full_path);

    // Read file using libc fopen/fread.
    const c_path = try c.alloc.dupeZ(u8, full_path);
    defer c.alloc.free(c_path);

    const f = std.c.fopen(c_path.ptr, "rb") orelse {
        return c.reportError("Failed to open file.", cx.node);
    };
    ...
    if (size < 0) {
        return c.reportError("Failed to get file size.", cx.node);
    }

    const data = try c.alloc.alloc(u8, @intCast(size));
    defer c.alloc.free(data);

    const n = std.c.fread(data.ptr, 1, data.len, f);
    if (n != data.len) {
        return c.reportError("Failed to read file.", cx.node);
    }
...
pub fn system(c: *cy.Chunk, ctx: *cy.ConstEvalContext) !cy.TypeValue {
    _ = c;
    const kind: SystemKind = switch (builtin.os.tag) {
        .linux => .linux,
        .macos => .macos,
        .windows => .windows,
