...
fn astringReplace(t: *cy.Thread, str: []const u8, needlev: cy.heap.Str, replacev: cy.heap.Str) !?cy.heap.Str {
    ...
    var aw: std.Io.Writer.Allocating = .initAligned(t.alloc, .@"4");
    defer aw.deinit();
    const newLen = try string.prepReplacement(str, needle, replacement, &aw.writer);
    const idxesBuf = aw.written();
    const numIdxes = idxesBuf.len / 4;
    if (numIdxes > 0) {
        if (replacev.ascii()) {
            const new = try t.heap.init_astr_undef(newLen);
            const newBuf = new.mutSlice();
            const idxes = @as([*]const u32, @ptrCast(@alignCast(idxesBuf.ptr)))[0..numIdxes];
