...
#[bind] fn @new_object_undef(type_id int, nbytes int) -> Ptr[void]

#[bind] fn @new_object_undef2(thread Thread, type_id int, nbytes int) -> Ptr[void]
...
fn @init_astr_undef(thread Thread, len int) -> str:
    buf := as[^StrBuffer] @new_object_undef2(thread, type.id(StrBuffer), 8 + len)
    buf.inner.length = len
    return {
        buf    = buf,
        ptr    = as *buf.inner.data,
