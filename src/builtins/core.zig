        ...
        pub fn genFunc(c: *cy.Chunk, ctx: *cy.ConstEvalContext) callconv(.c) cy.TypeValue {
            return @call(.always_inline, f, .{ c, ctx }) catch |err| {
                if (cy.compiler.dumpCompileErrorStackTrace and !C.silent()) {
                    if (@errorReturnTrace()) |__t| std.debug.dumpErrorReturnTrace(__t);
                }
...
pub fn start_thread(t: *cy.Thread) !C.Ret {
    ...
    const func_ret_size: usize = @intCast(t.param(i64));
    const entry_ret_size: usize = func_ret_size;
    const entry_ret_reg_size: usize = (entry_ret_size + 7) >> 3;

    // Frame layout from stack_ptr (callFunc2 will set fp = stack_ptr + entry_ret_reg_size):
    //   [0..entry_ret_reg_size-1]     : return value for @thread_entry
    //   [entry_ret_reg_size+0]        : call info (filled by callFunc2)
    //   [entry_ret_reg_size+1]        : return pc (filled by callFunc2)
    //   [entry_ret_reg_size+2]        : saved fp (filled by callFunc2)
    //   [entry_ret_reg_size+3]        : callee slot
    //   [entry_ret_reg_size+4]        : arg0: result_thread = parent thread
    //   [entry_ret_reg_size+5]        : arg1: func = user function
    //   [entry_ret_reg_size+6..]      : arg2: args tuple (func_args.len Value slots)
    const required = entry_ret_reg_size + 6 + func_args.len;
    try thread.stackEnsureTotalCapacityPrecise(required);

    // After potential stack grow, reset fp to new stack_ptr.
    thread.c.fp = thread.c.stack_ptr;

    thread.c.stack_ptr[entry_ret_reg_size + 4] = Value.initPtr(t);
    thread.c.stack_ptr[entry_ret_reg_size + 5] = func;
    @memcpy(thread.c.stack_ptr[entry_ret_reg_size + 6 .. entry_ret_reg_size + 6 + func_args.len], func_args);

    try t.c.vm.worker_pool.add_thread_task(thread, entry_func, future, entry_ret_size);
...
fn str_fmt_bytes(t: *cy.Thread) anyerror!C.Ret {
    ...
    const kind: NumberFormat = @enumFromInt(@as(u8, @intCast(t.param(i64))));
    if (kind == .asc) {
        ...
        defer aw.deinit();
        for (str) |byte| {
            try aw.writer.printInt(byte, base, .lower, .{ .width = width, .fill = '0' });
        }
        ret.* = try t.heap.init_astr(aw.written());
        return C.RetOk;
    }
}
...
fn raw_fmt(t: *cy.Thread) anyerror!C.Ret {
    ...
    const format: NumberFormat = @enumFromInt(@as(u8, @intCast(t.param(i64))));
...
pub fn int_fmt_eval(c: *cy.Chunk, ctx: *cy.ConstEvalContext) !cy.TypeValue {
    ...
    const format: NumberFormat = @enumFromInt(@as(u8, @intCast(ctx.args[1].asInt())));
...
fn int_fmt2(t: *cy.Thread) anyerror!Value {
    ...
    const format: NumberFormat = @enumFromInt(@as(u8, @intCast(t.getInt(1))));
