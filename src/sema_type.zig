...
pub fn ensure_resolved_type(c: *cy.Chunk, type_: *cy.Type, node: *ast.Node) anyerror!void {
    ...
    switch (type_.kind()) {
        .never, .eval_int, .eval_ref, .void, .func_ptr, .func_sym, .c_variadic, .generic_vector, .generic => {
...
pub fn resolveCustomTypeAlias(c: *cy.Chunk, sym: *cy.sym.TypeAlias) anyerror!void {
    ...
    const bind_name = name orelse b: {
        ...
        errdefer aw.deinit();
        try sema.writeBindName(&aw.writer, &sym.head);
        owned_name = true;
        break :b try aw.toOwnedSlice();
    };
    defer if (owned_name) c.alloc.free(bind_name);
...
pub fn resolve_cunion_type(c: *cy.Chunk, c_union: *cy.types.CUnion, node: *ast.Node) !void {
    ...
    for (decl.cases.slice(), 0..) |case, i| {
        ...
        const case_sym = try c.declareUnionCase(@ptrCast(c_union.base.sym()), case_name, @ptrCast(c_union), @intCast(i), payload_t, @ptrCast(case));
        cases[i] = .{
            .sym = case_sym,
...
pub fn resolveStructFields(c: *cy.Chunk, struct_t: *cy.types.Struct, decl: *ast.StructDecl) !void {
    ...
    for (decl.fields.slice(), 0..) |field, i| {
        ...
        if (field.embedded) {
            if (field_t.kind() != .struct_t) {
                return c.reportErrorFmt("Embedded field must be a struct type, got {s}", &.{v(@tagName(field_t.kind()))}, @ptrCast(field));
            }
...
fn resolveCopiedType(c: *cy.Chunk, sym: *cy.sym.TypeSym, src_t: *cy.Type, node: *ast.Node) !void {
    switch (src_t.kind()) {
        .partial_vector, .vector, .pointer, .hostobj, .int => {},
        .struct_t => {
