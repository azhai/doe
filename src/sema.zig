...
pub fn semaStmt(c: *cy.Chunk, node: *ast.Node) !void {
    ...
    switch (node.type()) {
        ...
        .op_assign_stmt => {
            ...
            switch (stmt.op) {
                .bitwiseOr, .bitwiseAnd, .bitwiseLeftShift, .bitwiseRightShift, .bitwiseXor, .star, .slash, .percent, .pow, .plus, .minus => {},
                else => {
        ...
        .trait_decl, .struct_decl, .cstruct_decl, .cunion_decl, .passStmt, .const_decl, .global_decl, .custom_type_decl, .type_alias_decl, .type_const_decl, .use_alias, .import_stmt, .template, .enumDecl, .funcDecl => {
            // Nop.
        },
        .for_iter_stmt => {
...
fn semaReturnExprStmt(c: *cy.Chunk, child: *ast.Node, ret_opt: ?*cy.Type, node: *ast.Node) !*cy.Type {
    ...
    // Move local variable on return instead of copying, but only if the
    // local was declared in the same block (avoids killing outer-scope
    // locals returned from conditional branches).
    if (expr.resType == .value_local and expr.data.value_local >= c.block().varStart) {
        const local = &c.varStack.items[expr.data.value_local];
...
fn semaIfBlock(c: *cy.Chunk, cond_n: *ast.Node, stmts: []const *ast.Node, outer: usize, is_last: bool, end_reachable: *bool, node: *ast.Node) !void {
...
fn semaIfUnwrapBlock2(c: *cy.Chunk, opt: ExprResult, opt_n: *ast.Node, opt_unwrap: ?*ast.Node, body: *const fn (*cy.Chunk, u32, ExprResult, *anyopaque) anyerror!void, body_data: *anyopaque, outer: usize, is_last: bool, node: *ast.Node) !void {
...
fn semaIfUnwrapBlock(c: *cy.Chunk, opt_n: *ast.Node, opt_unwrap: ?*ast.Node, stmts: []const *ast.Node, outer: usize, is_last: bool, end_reachable: *bool, node: *ast.Node) !void {
...
pub fn ensure_resolved_func(c: *cy.Chunk, func: *cy.Func, resolve_new_types: bool, node: ?*ast.Node) !void {
    ...
    switch (func.type) {
        ...
        .reserved, .generic, .userFunc, .hostFunc, .extern_, .vm_extern_variant, .host_builtin, .host_const_eval => {
...
pub fn resolveHostVar(c: *cy.Chunk, sym: *cy.sym.HostVar) !void {
    ...
    var aw: std.Io.Writer.Allocating = .init(c.alloc);
    defer aw.deinit();
    try writeBindName(&aw.writer, &sym.head);
    const bind_name = aw.written();
...
pub const NameResult = struct {
    ...
    fn initCtValue2(val_t: *cy.Type, val: cy.Value) NameResult {
        return .{ .type = .ct_value, .data = .{
            .ct_value = cy.TypeValue.init(val_t, val),
        } };
    }
...
pub fn sema_program(c: *cy.Chunk, opt_main_func: ?*cy.Func) !*ir.Stmt {
    ...
    if (opt_main_func) |main_func| {
        for (c.ast.root.?.stmts.slice()) |stmt| {
            switch (stmt.type()) {
                .trait_decl, .struct_decl, .cstruct_decl, .cunion_decl, .passStmt, .const_decl, .global_decl, .custom_type_decl, .type_alias_decl, .template, .enumDecl, .use_alias, .import_stmt, .funcDecl, .ct_stmt => {},
                else => {
...
fn semaSwitchBody(c: *cy.Chunk, info: *SwitchInfo, block: *ast.SwitchBlock, control: ExprResult) !void {
    ...
    var cases_buf: std.ArrayListUnmanaged(*ir.Expr) = .empty;
...
fn semaSwitchCase(c: *cy.Chunk, info: *SwitchInfo, case: *ast.CaseStmt, control: ExprResult) !*ir.Expr {
    ...
    var conds_buf: std.ArrayListUnmanaged(*ir.Expr) = .empty;
...
pub fn semaTupleInit(c: *cy.Chunk, type_: *cy.Type, fields: []const cy.types.Field, lit: *ast.InitLit) !ExprResult {
    ...
    const init = try c.ir.newExpr(.init, type_, @ptrCast(lit), .{
        .nargs = @as(u8, @intCast(fields.len)),
        .args = args.ptr,
    });
...
pub fn semaCallFuncSymRec(c: *cy.Chunk, sym: *cy.sym.FuncSym, rec: *ast.Node, rec_res: ExprResult, args: []const *ast.Node, req_value: bool, node: *ast.Node) !ExprResult {
...
pub fn semaCallFuncSym2(c: *cy.Chunk, sym: *cy.sym.FuncSym, arg1_n: *ast.Node, arg1_res: ExprResult, arg2_n: *ast.Node, arg2_res: ExprResult, node: *ast.Node) !ExprResult {
...
pub fn semaUnExpr(c: *cy.Chunk, unary: *ast.Unary, expr: Cstr) !ExprResult {
    ...
    switch (unary.op) {
        ...
        .bitwiseNot => {
            ...
            switch (child.type.id()) {
                bt.R8, bt.R16, bt.R32, bt.R64, bt.I8, bt.I16, bt.I32, bt.I64 => {
...
fn sema_bin_expr3(c: *cy.Chunk, left: ExprResult, left_n: *ast.Node, right: ExprResult, right_n: *ast.Node, op: cy.BinaryExprOp, node: *ast.Node) !ExprResult {
    switch (left.type.id()) {
        ...
        bt.F64, bt.F32, bt.I8, bt.I16, bt.I32, bt.I64 => {
            const cond: ir.Condition = switch (op) {
                .greater => .gt,
                .greater_equal => .ge,
                .less => .lt,
...
pub fn semaBinExpr(c: *cy.Chunk, expr: Cstr, left_n: *ast.Node, op: cy.BinaryExprOp, right_n: *ast.Node, node: *ast.Node) !ExprResult {
    switch (op) {
        ...
        .bitwiseAnd, .bitwiseOr, .bitwiseXor, .bitwiseLeftShift, .bitwiseRightShift, .star, .slash, .percent, .pow, .plus, .minus => {
            switch (left_n.type()) {
                .binLit, .octLit, .hexLit, .decLit => {
...
pub fn semaCompare(c: *cy.Chunk, left_n: *ast.Node, left: ExprResult, right_n: *ast.Node, right: ExprResult, node: *ast.Node) !ExprResult {
    var simple_cmp = true;
    switch (left.type.id()) {
        bt.R8, bt.I8, bt.I16, bt.I32, bt.I64, bt.F64, bt.Bool => {},
        else => {
            switch (left.type.kind()) {
                .pointer, .enum_t, .int => {},
                .option, .choice, .result, .struct_t => {
                    simple_cmp = false;
                },
                else => {},
            }
...
pub fn semaNopFmt(c: *cy.Chunk, comptime format: []const u8, args: anytype, node: *ast.Node) !void {
    ...
    var aw: std.Io.Writer.Allocating = .init(c.alloc);
    defer aw.deinit();
    try aw.writer.print(format, args);
    try c.ir.buf.appendSlice(c.alloc, aw.written());
...
pub const Sema = struct {
    ...
    pub fn init(alloc: std.mem.Allocator, compiler: *cy.Compiler) !Sema {
        var new = Sema{
            ...
            .func_sig_map = .empty,
            .types = .empty,
            .partial_struct_layouts = .empty,
            .symbols = .empty,
            .symbols_map = .empty,
            .vm_c_variadic_funcs = .empty,
        };
...
pub fn semaCZero(c: *cy.Chunk, type_: *cy.Type, node: *ast.Node) !ExprResult {
    switch (type_.id()) {
        bt.R8, bt.I8 => return c.semaConst8(0, type_, node),
        bt.Bool => return c.semaFalse(node),
        bt.R16, bt.I16 => return c.semaConst16(0, type_, node),
        bt.R32, bt.F32, bt.I32 => return c.semaConst32(0, type_, node),
        bt.R64, bt.I64, bt.F64 => return c.semaConst(0, type_, node),
        else => {
            switch (type_.kind()) {
                .struct_t => {
