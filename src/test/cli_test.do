...
fn runExpectErr(src, expErr):
    os.writeFile('temp.do', src)
    var doe = os.exePath()
    var res = os.execCmd([ doe, 'temp.do'])
    if res.exited != 0:
        print res.out
        print res.err
    t.eq(res.exited, 0)
...
fn runPipeInput(cmd, src):
    ...
    let res = none
    if os.system == 'windows':
      res = os.execCmd([ 'powershell', '-c', '$(cmd) | $(doe) temp.do' ])
    else:
      res = os.execCmd([ '/bin/bash', '-c', '$(cmd) | $(doe) temp.do' ])
    if res.exited != 0:
        print res.out
        print res.err
    t.eq(res.exited, 0)
