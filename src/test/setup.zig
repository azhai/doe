...
pub const VMrunner = struct {
    ...
    pub fn init(enable_cli: bool) !VMrunner {
        ...
        test_vm = vm;
        if (enable_cli) {
            try cli.init_cli(vm, t.alloc, std.Io.failing, &.{});
        } else {
    ...
    pub fn eval(run: *VMrunner, config: Config, opt_src: ?[]const u8, optCb: ?*const fn (*VMrunner, EvalResult) anyerror!void) !void {
        ...
        // Set and restore cwd.
        var cwdBuf: [4096:0]u8 = undefined;
        const cwd = std.c.getcwd(&cwdBuf, cwdBuf.len) orelse return error.GetCwdFailed;
        const cwd_z: [*:0]const u8 = @ptrCast(cwd);
        if (config.chdir) |chdir| {
            const chdir_z = t.alloc.dupeZ(u8, chdir) catch return error.OutOfMemory;
            defer t.alloc.free(chdir_z);
            _ = std.c.chdir(chdir_z.ptr);
        }
        defer {
    ...
    while (true) {
        if (std.mem.indexOf(u8, cur, "@AbsPath(")) |idx| {
            if (std.mem.indexOfScalarPos(u8, cur, idx, ')')) |endIdx| {
                ...
                const basePath_z = std.c.getcwd(&cwd_buf, cwd_buf.len) orelse ".";
                const basePath: []const u8 = std.mem.sliceTo(basePath_z, 0);
