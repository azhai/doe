...
fn getPrevFrame(vm: *cy.VM, stack: []const Value, fp: u32) PcFpOff {
    const prev_pc = getInstOffset(vm.c.ops, stack[fp + 2].getRetPcPtr()) - stack[fp + 1].call_info.call_inst_off;
    const prev_fp = getStackOffset(stack.ptr, stack[fp + 3].getRetFramePtr());
...
pub fn stackGrowTotalCapacityPrecise(self: *cy.VM, newCap: usize) !void {
    if (self.alloc.resize(self.c.getStack(), newCap)) {
        ...
        if (builtin.is_test or cy.Trace) {
            ...
            @memset(self.c.getStack(), Value.initPtr(&DummyHeapObject));
        }
    }
}
...
fn growStackPrecise(vm: *cy.VM, newCap: usize) !void {
    if (vm.alloc.resize(vm.c.getStack(), newCap)) {
        ...
        while (curFpOffset != 0) {
            const prevFpOffset = getStackOffset(vm.c.stack, newStack[curFpOffset + 3].getRetFramePtr());
            newStack[curFpOffset + 3].setRetFramePtr(newStack.ptr + prevFpOffset);
