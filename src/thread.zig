...
pub const Thread = struct {
    ...
    pub fn init(self: *Thread, id: usize, alloc: std.mem.Allocator, vm: *cy.VM) !void {
        self.* = .{
            .heap = try cy.Heap.init(vm, self, alloc, vm.sema),
            .alloc = alloc,
            .c = .{
                .id = id,
                .vm = vm,
        ...
        self.c.fp_end = self.c.stack_ptr;
        self.c.pc = undefined;

        if (cy.Trace) {
            self.c.trace = try alloc.create(vmc.TraceInfo);
            self.c.trace_pc = null;
            self.c.trace_fp = null;
        } else {
            self.c.trace = null;
            self.c.trace_pc = null;
            self.c.trace_fp = null;
    ...
    pub fn callFunc2(self: *Thread, ret_: [*]Value, ret_size: u16, func_ptr: Value) ![*]Value {
        ...
        if (host_func) {
            ...
            ret_[ret_size + 1] = Value.initRetPcPtr(cur_pc);
            ret_[ret_size + 2] = Value.initRetFramePtr(self.c.fp);
            ...
            ret_[ret_size + 1] = Value.initRetPcPtr(&end);
            ret_[ret_size + 2] = Value.initRetFramePtr(self.c.fp);
    ...
    pub fn getTraceFiberContext(self: *const cy.Thread) PcFpOff {
        return .{
            .pc = self.c.trace_pc orelse self.c.pc,
            .fp = getStackOffset(self.c.stack_ptr, self.c.trace_fp orelse self.c.fp),
        };
    }
    ...
    pub fn callUnion(self: *Thread, callee: Value, args: []const Value) !cy.Value {
        ...
        switch (func.kind) {
            .pinned_closure, .closure => {
                ...
                new_fp[2] = Value.initRetPcPtr(cur_pc);
                new_fp[3] = Value.initRetFramePtr(cur_fp);
                new_fp[4] = callee;
            ...
            .host => {
                ...
                cur_fp[ret_ + 2] = Value.initRetPcPtr(cur_pc);
                cur_fp[ret_ + 3] = Value.initRetFramePtr(cur_fp);
    ...
    pub fn callPtrInst(self: *Thread, pc: [*]cy.Inst, fp: [*]Value, base: u16) !PcFp {
        ...
        if (host_func) {
            ...
            base_ptr[1] = Value.initRetPcPtr(pc);
            base_ptr[2] = Value.initRetFramePtr(fp);
            ...
            base_ptr[1] = Value.initRetPcPtr(pc + cy.bytecode.CallPtrInstLen);
            base_ptr[2] = Value.initRetFramePtr(fp);
            return PcFp{
    ...
    pub fn callTraitSymInst(
        ...
        if (host_func) {
            ...
            fp[base + 1] = Value.initRetPcPtr(pc);
            fp[base + 2] = Value.initRetFramePtr(fp);
            ...
            base_ptr[1] = Value.initRetPcPtr(pc + cy.bytecode.CallTraitInstLen);
            base_ptr[2] = Value.initRetFramePtr(fp);
            return PcFp{
    ...
    pub fn callUnionInst(self: *Thread, pc: [*]cy.Inst, fp: [*]Value, base: u16) !PcFp {
        ...
        switch (func.kind) {
            .pinned_closure, .closure => {
                ...
                base_ptr[1] = Value.initRetPcPtr(pc + cy.bytecode.CallUnionInstLen);
                base_ptr[2] = Value.initRetFramePtr(fp);
                return PcFp{
    ...
    fn growStackPrecise(self: *Thread, newCap: usize) !void {
        if (self.alloc.resize(self.c.stack(), newCap)) {
            ...
            const ret_size = @as(*align(1) const u16, @ptrCast(self.c.pc + 1)).*;
            self.c.fp += ret_size;
        } else {
            ...
            while (frame.frame_type() != .init) {
                ...
                newStack[base_off + 2].setRetFramePtr(newStack.ptr + prev_fp_off);
                pc = prev.pc;
                fp = prev_fp_off;
    ...
    pub fn unwindStack(self: *cy.Thread, tstack: []Value, cx: PcFpOff) !PcFpOff {
        ...
        while (true) {
            ...
            if (frame_t == .gen) {
                ...
                pc = tstack[fp + 2].getRetPcPtr() - tstack[fp + 1].call_info.call_inst_off;
    ...
    pub fn dump_stats(self: *const Thread) !void {
        ...
        const trace = self.c.trace.?;
        try w.print("main ops evaled: {}\n", .{trace.totalOpCounts});
        std.sort.pdq(vmc.OpCount, &trace.opCounts, {}, S.opCountLess);
        var i: u32 = 0;

        while (i < vmc.NumCodes) : (i += 1) {
            if (trace.opCounts[i].count > 0) {
                const op: cy.OpCode = @enumFromInt(trace.opCounts[i].code);
                try w.print("\t{s} {}\n", .{ @tagName(op), trace.opCounts[i].count });
            }
        }
...
const FrameInfo = struct {
    ...
    pub fn prev_context(self: *const FrameInfo) PcFp {
        ...
        const ret_pc: ?[*]cy.Inst = base[1].getRetPcPtr();
        if (ret_pc) |pc| {
            return PcFp{ .pc = pc - base[0].call_info.call_inst_off, .fp = base[2].getRetFramePtr() };
        } else {
            return PcFp{ .pc = null, .fp = base[2].getRetFramePtr() };
        }
    }
};
...
pub const CallInfo = packed struct(u64) {
