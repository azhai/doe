...
pub const FFI = struct {
    ...
    fn toElemCType(self: *FFI, vm: *cy.VM, spec: Value) !CType {
        ...
        //     const sym = try @enumFromInt(spec.asSymbol() catch unreachable);
...
pub fn ffiBindLib(t: *cy.Thread, config: BindLibConfig, _: *DynLib) !c.Ret {
    _ = config;
    _ = t;
    // C bindings backend (cgen + tcc) was removed. ffiBindLib is a no-op.
    return c.RetOk;
}
...
pub fn ffiBindCallback(vm: *cy.VM) anyerror!Value {
    ...
    var csrc: std.ArrayListUnmanaged(u8) = .empty;
