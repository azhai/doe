...
pub const Compiler = struct {
    ...
    fn compileInner(self: *Compiler, uri: []const u8, src_opt: ?[]const u8, config: C.CompileConfig) !CompileResult {
        ...
        const mod = self.main_chunk.sym.getMod();
        if (mod.getSym("main")) |main_func| {
                        ...
                        for (impls, 0..) |*impl, j| {
                            ...
                            for (trait_members, 0..) |member, i| {
                                const func = type_.get_trait_method_impl(member) orelse {
                                    ...
                                    return chunk.reportErrorFmt("`{}` does not implement `fn {}{}` from `{}`.", &.{ v(type_name), v(member.func.name()), v(sig_str), v(impl.trait.base.name()) }, @ptrCast(sym.decl.?.cast(.struct_decl).impls.ptr[j].trait));
                                };
                                funcs[i] = func;
                            }
...
/// Since core types are declared in the same builtins.do,
