...
export fn cl_vm_dump_bytecode_to_file(vm: *cy.VM, c_path: [*:0]const u8) bool {
    const path = std.mem.sliceTo(c_path, 0);

    var path_buf: [4096]u8 = undefined;
    const path_z = std.fmt.bufPrintZ(&path_buf, "{s}", .{path}) catch return false;
    const f = std.c.fopen(path_z.ptr, "wb") orelse return false;
    defer _ = std.c.fclose(f);

    var lw = std.Io.Writer.Allocating.init(vm.alloc);
    defer lw.deinit();
    cy.debug.dumpBytecodeToWriter(vm, .{}, &lw.writer) catch return false;

    const contents = lw.written();
    const written = std.c.fwrite(contents.ptr, 1, contents.len, f);
    return written == contents.len;
}
...
test "meta constants" {
    ...
    , &eval_res);
    try zt.eq(C.Success, res);
    const exp_system: meta_mod.SystemKind = switch (builtin.os.tag) {
        .linux => .linux,
        .macos => .macos,
        .windows => .windows,
    ...
    , &eval_res);
    try zt.eq(C.Success, res);
    const exp_endian: Endian = switch (builtin.cpu.arch.endian()) {
        .little => .little,
        .big => .big,
    };
