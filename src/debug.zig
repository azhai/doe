...
pub fn compactToStackFrame(vm: *cy.VM, stack: []const cy.Value, frame: vmc.CompactFrame) !C.StackFrame {
    if (frame.pc) |pc| {
        ...
        const call_pc = stack[frame.fpOffset + 1].getRetPcPtr() - stack[frame.fpOffset].call_info.call_inst_off;
...
pub fn dumpBytecode(vm: *cy.VM, opts: DumpBytecodeOptions) !void {
    ...
    defer lw.deinit();
    try dumpBytecodeToWriter(vm, opts, &lw.writer);
    vm.log(lw.written());
}

pub fn dumpBytecodeToWriter(vm: *cy.VM, opts: DumpBytecodeOptions, w: *std.Io.Writer) !void {
    _ = opts;
    ...
    for (vm.compiler.chunks.items, 0..) |c, chunk_id| {
        ...
        // Build func pc map.
        var funcs: std.AutoHashMapUnmanaged(*anyopaque, *cy.Func) = .empty;
