...
pub const Value = extern union {
    ...
    pub inline fn asPtr(self: *const Value, comptime Ptr: type) Ptr {
        ...
        // Use optional intermediate to avoid panic when ptr is 0 (none/null).
        const opt: ?*anyopaque = @ptrFromInt(self.ptr);
        return @ptrCast(@alignCast(opt));
    }
    ...
    pub inline fn asHeapObject(self: *const Value) *cy.HeapObject {
        // NOTE: Access `ptr` since the upper bits may be undefined for 32-bit pointers.
        return @ptrCast(@alignCast(@as(*anyopaque, @ptrFromInt(self.ptr))));
    }
    ...
    pub inline fn initRetPcPtr(ptr: [*]cy.Inst) Value {
        return .{ .retPcPtr = @intFromPtr(ptr) };
    }

    pub inline fn initRetFramePtr(ptr: [*]Value) Value {
        return .{ .retFramePtr = @intFromPtr(ptr) };
    }

    pub inline fn getRetPcPtr(self: *const Value) [*]cy.Inst {
        return @ptrFromInt(self.retPcPtr);
    }

    pub inline fn getRetFramePtr(self: *const Value) [*]Value {
        return @ptrFromInt(self.retFramePtr);
    }

    pub inline fn setRetPcPtr(self: *Value, ptr: [*]cy.Inst) void {
        self.retPcPtr = @intFromPtr(ptr);
    }

    pub inline fn setRetFramePtr(self: *Value, ptr: [*]Value) void {
        self.retFramePtr = @intFromPtr(ptr);
    }
