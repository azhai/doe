...
pub const VM = struct {
    ...
    pub fn init(self: *VM, alloc: std.mem.Allocator) !void {
        ...
        self.* = .{
            ...
            .task_nodes = .empty,
            .vtables = .empty,
            .env_local_saves = .empty,
            .env_local_saves_map = .empty,
            .dyn_libs = .empty,
            .tcc_state = null,
        };
    ...
    pub fn deinit(self: *VM, reset: bool) void {
        ...
        if (!reset) {
            if (self.data.get("builtins")) |data_ptr| {
                const data: *cy.core.BuiltinsData = @ptrCast(@alignCast(data_ptr));
    ...
    pub fn eval(self: *VM, src_uri: []const u8, src: ?[]const u8, config: C.EvalConfig) !C.EvalResult {
        ...
        if (config.backend == C.BackendJIT) {
            ...
            if (cy.Trace) {
                const trace = self.main_thread.c.trace.?;
                var i: u32 = 0;
                while (i < trace.opCounts.len) : (i += 1) {
                    trace.opCounts[i] = .{
                        .code = i,
                        .count = 0,
                    };
                }
                trace.totalOpCounts = 0;
...
pub var traceObjRetains: std.AutoHashMapUnmanaged(u32, u32) = .empty;
