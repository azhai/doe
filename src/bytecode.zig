...
pub const ByteCodeBuffer = struct {
    ...
    pub fn init(alloc: std.mem.Allocator, vm: *cy.VM) ByteCodeBuffer {
        const new = ByteCodeBuffer{
            ...
            .debugTable = .empty,
            .func_table = .empty,
            .const_string_map = .empty,
            .ptr_table = .empty,
            .ptr_layouts = .empty,
            .ptr_layout_map = .empty,
            .ptr_layout_builder = .empty,
            .markers = .empty,
            .labels = .empty,
        };
        return new;
...
pub fn write_inst(vm: *cy.VM, w: *std.Io.Writer, code: OpCode, chunk_id: ?usize, pc_off: ?usize, pc: [*]const Inst, opts: DumpInstOptions) !void {
    ...
    switch (code) {
        ...
        .or_, .lsl, .feq, .feq32, .lt, .flt, .flt32, .le, .fle, .fle32, .gt, .fgt, .fgt32, .ge, .fge, .fge32, .fdiv, .fdiv32, .idiv, .div, .fmul, .fmul32, .imul, .mul, .fsub, .fsub32, .sub, .fadd, .fadd32, .add => {
            const dst = pc[1].val;
            const lhs = pc[2].val;
            const rhs = pc[3].val;
            const op: []const u8 = switch (code) {
                ...
                .gt, .fgt32, .fgt => ">",
                .ge, .fge32, .fge => ">=",
                .fdiv, .fdiv32, .idiv, .div => "/",
                .fmul, .fmul32, .imul, .mul => "*",
                .fsub, .fsub32, .sub => "-",
                .fadd, .fadd32, .add => "+",
        ...
        .call => {
            ...
            const func_id: u32 = @as(*align(2) const u32, @ptrFromInt(func_pc - 4)).*;
        ...
        .call_trait => {
            const base = pc[1].val;
            const vtable_idx = @as(*align(1) const u16, @ptrCast(pc + 3)).*;
            try w.print("?..%{} = %{}.vtable[{}](%{}..)", .{
        ...
        .nops => {
            const ptr_len = @as(*align(2) const u64, @ptrCast(pc + 1)).*;
        ...
        .const_str => {
            const dst = pc[1].val;
            const ptr_len = @as(*align(2) const u64, @ptrCast(pc + 2)).*;
            const ptr: [*]u8 = @ptrFromInt(@as(usize, @intCast(ptr_len & 0xffffffffffff)));
            const len_: usize = @intCast(ptr_len >> 48);
        ...
        .const_64 => {
            const dst = pc[1].val;
            const val = @as(*align(2) const u64, @ptrCast(pc + 2)).*;
            try w.print("%{} = {}", .{ dst, val });
        },
        .mov_4, .mov_3, .mov_2, .mov => {
            const dst = pc[1].val;
            const src = pc[2].val;
            try w.print("%{} = %{}", .{ dst, src });
        },
        ...
        .is_zero => {
            const dst = @as(*align(1) const u16, @ptrCast(pc + 1)).*;
            const val = @as(*align(1) const u16, @ptrCast(pc + 3)).*;
        ...
        .trait => {
            const src = pc[1].val;
            const type_id = @as(*align(1) const u16, @ptrCast(pc + 2)).*;
            const vtable = @as(*align(1) const u16, @ptrCast(pc + 4)).*;
            const dst = pc[6].val;
        ...
        .new => {
            const dst = @as(*align(1) const u16, @ptrCast(pc + 1)).*;
            const type_id = @as(*align(1) const u16, @ptrCast(pc + 3)).*;
            const name = try vm.sema.allocTypeName(vm.sema.types.items[type_id]);
            defer vm.alloc.free(name);
            const size = @as(*align(1) const u16, @ptrCast(pc + 5)).*;
            try w.print("%{} = {s}{{size={}}}", .{
                dst, name, size,
            });
        },
        .retain => {
            const reg = @as(*align(1) const u16, @ptrCast(pc + 1)).*;
            try w.print("%{}.rc += 1", .{reg});
        },
        .release_opt, .release => {
            const reg = @as(*align(1) const u16, @ptrCast(pc + 1)).*;
            const jump = @as(*align(1) const i16, @ptrCast(pc + 3)).*;
        ...
        .store_4w, .store_3w, .store_2w, .store_64, .store_32, .store_16, .store_8 => {
        ...
        .load_2w, .load_3w, .load_4w, .load_8, .load_32, .load_16, .load_64 => {
            const dst = pc[1].val;
            const src = pc[2].val;
            const src_off = pc[3].val;
        ...
        .ret_gen => {
            ...
            try w.print("ret gen(type={}, %{}..%{}, resume_off={}, ret_size={})", .{ type_id, ret_size + 4, reg_end, resume_off, ret_size });
        },
        .gen_next => {
            const ret = @as(*align(1) const u16, @ptrCast(pc + 1)).*;
            const base = @as(*align(1) const u16, @ptrCast(pc + 3)).*;
            const gen = @as(*align(1) const u16, @ptrCast(pc + 5)).*;
        ...
        .memsetz => {
            const dst_ptr = @as(*align(1) const u16, @ptrCast(pc + 1)).*;
            const offset = @as(*align(1) const u16, @ptrCast(pc + 3)).*;
            const len = @as(*align(1) const u16, @ptrCast(pc + 5)).*;
            try w.print("[%{}+{}..{}] = 0", .{ dst_ptr, offset, offset + len });
        },
        .lnot => {
            const dst = @as(*align(1) const u16, @ptrCast(pc + 1)).*;
            const cond = @as(*align(1) const u16, @ptrCast(pc + 3)).*;
            try w.print("%{} = !%{}", .{ dst, cond });
        },
        .sext => {
            const dst = @as(*align(1) const u16, @ptrCast(pc + 1)).*;
            const src = @as(*align(1) const u16, @ptrCast(pc + 3)).*;
            const bits = pc[5].val;
        ...
        .fn_union => {
            const dst = @as(*align(1) const u16, @ptrCast(pc + 1)).*;
            const src = @as(*align(1) const u16, @ptrCast(pc + 3)).*;
            const type_id = @as(*align(1) const u16, @ptrCast(pc + 5)).*;
...
pub fn getInstLenAt(pc: [*]const Inst) u8 {
    switch (pc[0].opcode()) {
        .end, .ret_0, .ret, .trap_debug, .trap => {
            return 1;
        },
        .retain_nz => {
            return 2;
        },
        .await_op, .retain, .true, .false, .dtor_str, .ret_n, .call_ptr, .call_union, .jump => {
            return 2;
        },
        .const_16s, .const_16 => {
            return 3;
        ...
        .gen_next, .gen_end, .fn_union, .lsl, .lsr, .fadd, .fadd32, .fsub, .fsub32, .fmul, .fmul32, .fdiv, .fdiv32, .fmod, .fmod32, .add, .sub, .imul, .mul, .idiv, .div, .pow, .imod, .mod, .and_, .or_, .xor, .cmp_8, .cmp, .cmp_str, .castAbstract, .memsetz, .feq32, .feq, .flt, .fgt, .fle, .fge, .flt32, .fgt32, .fle32, .fge32, .lt, .gt, .le, .ge, .ult, .ugt, .ule, .uge, .add_i16, .mov_n, .new, .const_32, .load_8, .load_16, .load_32, .load_64, .load_2w, .load_3w, .load_4w, .store_8, .store_16, .store_32, .store_64, .store_2w, .store_3w, .store_4w => {
            return 4;
        },
        .ret_gen, .cast => {
            return 5;
        },
        .unwrap_addr, .fn_vm, .fn_host, .trait, .load_n, .store_n, .call, .call_host, .nops => {
            return 5;
        },
        .const_64 => {
            return 6;
