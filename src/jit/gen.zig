...
pub const Reloc = struct {
    ...
    data: union {
        ...
        jit_entry: struct { chunk: *cy.Chunk, bc_pc: u32, func: *cy.Func },
    },
};
...
pub const CodeBuffer = struct {
    ...
    pub fn unset_executable(self: *CodeBuffer) !void {
        ...
        if (builtin.os.tag == .windows) {
            ...
            const addr = @as([*]align(std.heap.page_size_min) u8, @ptrCast(self.buf.items.ptr));
            const aligned_len = std.mem.alignForward(usize, self.buf.capacity, page_size);
            const prot: std.c.PROT = .{ .READ = true, .WRITE = true };
            if (std.c.mprotect(addr, aligned_len, prot) != 0) {
    ...
    pub fn set_executable(self: *CodeBuffer) !void {
        ...
        const page_size = std.heap.page_size_min;
        const addr = @as([*]align(std.heap.page_size_min) u8, @ptrCast(self.final_buf.ptr));
        const aligned_len = std.mem.alignForward(usize, self.final_buf.len, page_size);
        const prot: std.c.PROT = .{ .READ = true, .EXEC = true };
        if (std.c.mprotect(addr, aligned_len, prot) != 0) {
            return error.MProtectFailed;
        }
        if (builtin.os.tag == .windows) {
...
fn genStmt(c: *cy.Chunk, idx: u32) anyerror!void {
    ...
    if (cy.Trace and c.proc_stack.items.len > 0) {
        if (c.unwind_entry_stack.items.len != tempRetainedStart) {
            return c.reportErrorFmt("Expected {} unwindable retained temps, got {}", &.{ v(tempRetainedStart), v(c.unwind_entry_stack.items.len) }, node);
        }

        if (bc.numSlots(c) != slot_count) {
            return c.reportErrorFmt("Expected {} slots, got {}", &.{ v(slot_count), v(bc.numSlots(c)) }, node);
        }
    }
    ...
    while (@intFromPtr(pc) < @intFromPtr(end)) {
        ...
        switch (pc[0].opcode()) {
            ...
            .call => {
                ...
                const func_id: u32 = @as(*align(2) const u32, @ptrFromInt(func_vm_pc - 4)).*;
