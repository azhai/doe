...
/// Replacement for the removed `getEnvVarOwned`.
fn getEnvVarOwned(allocator: std.mem.Allocator, name: []const u8) error{ OutOfMemory, EnvironmentVariableNotFound }![]u8 {
    var name_buf: [256]u8 = undefined;
    const name_z = std.fmt.bufPrintZ(&name_buf, "{s}", .{name}) catch return error.OutOfMemory;
...
pub fn open(allocator: std.mem.Allocator, folder: KnownFolder, args: std.Io.Dir.OpenOptions) (std.Io.Dir.OpenError || Error)!?std.Io.Dir {
    _ = args;
    const path_or_null = try getPath(allocator, folder);
    if (path_or_null) |path| {
        defer allocator.free(path);

        // Std.Io.Dir requires an Io instance; this is a stub.
        return null;
    } else {
        return null;
    }
...
pub fn getPath(allocator: std.mem.Allocator, folder: KnownFolder) Error!?[]const u8 {
    if (folder == .executable_dir) {
        ...
        return allocator.dupe(u8, dir) catch return error.OutOfMemory;
    }
    ...
    switch (builtin.os.tag) {
            ...
            switch (folder_spec) {
                ...
                .by_env => |env_path| {
                    if (env_path.subdir) |sub_dir| {
                        const root_path = getEnvVarOwned(arena.allocator(), env_path.env_var) catch |err| switch (err) {
                            error.EnvironmentVariableNotFound => return null,
                            error.OutOfMemory => |e| return e,
                        };
                        return try std.fs.path.join(allocator, &[_][]const u8{ root_path, sub_dir });
                    } else {
                        return getEnvVarOwned(allocator, env_path.env_var) catch |err| switch (err) {
                            error.EnvironmentVariableNotFound => return null,
                            error.OutOfMemory => |e| return e,
                        };
                    }
        ...
        .macos => {
            ...
            switch (mac_folder_spec.get(folder)) {
                ...
                .suffix => |s| {
                    const home_dir = if (std.c.getenv("HOME")) |home|
                        std.mem.span(home)
                    else
                        return null;
...
fn getPathXdg(allocator: std.mem.Allocator, arena: *std.heap.ArenaAllocator, folder: KnownFolder) Error!?[]const u8 {
    ...
    if (@hasDecl(root, "known_folders_config") and root.known_folders_config.xdg_force_default) {
        if (folder_spec.default) |default| {
            if (default[0] == '~') {
                const home = getEnvVarOwned(arena.allocator(), "HOME") catch null orelse return null;
    ...
    const env_opt = env_opt: {
        if (getEnvVarOwned(arena.allocator(), folder_spec.env.name) catch null) |env_opt| break :env_opt env_opt;
...
test "open each known folders" {
    inline for (std.meta.fields(KnownFolder)) |fld| {
        ...
        if (dir_or_null) |_| {
            // dir.close() requires Io in Zig 0.16; leak in test is acceptable.
        }
    }
}
