...
Doe is a funny and useful programming language.
...
Doe provides memory safety by default by providing structured memory semantics.
Manual memory is also supported but it's forbidden in safe mode.

## Structured memory.
Doe supports value ownership, borrowing, and deinitializers.
...
### ARC.
Objects in Doe are reference counted. Each shared reference retains the count upon initialization and releases the count when it goes out of scope. The value that the reference points to is deinitialized and freed when the reference count reaches zero. This is also known as ARC (automatic reference counting).
...
### Growable stacks.
When running on Doe's VM, threads can grow their stack on demand.
...
[`libdoe`](#libdoe) allows an embedder to register these hooks.
Many of the builtins in Doe core and std modules are implemented using these bind hooks.
...
Modules have their own namespace of static symbols. By default, importing another Doe script returns a module with its declared symbols.
...
## Builtin modules.
Builtin and std modules come with Doe. See the [API docs](api.html).
...
When importing a URL without a file name, Doe's CLI will look for a `mod.cy` from the path instead.
...
This table maps Doe to C primitive types:
...
## Binding to C.
Doe supports binding to an existing C ABI compatible library at runtime.
...
4. When embedding [`libdoe`](#libdoe), compiler hooks can bind VM or C functions with the host language.

### Static binding.
When building a standalone executable, the C library's source or static library can be included for compilation. The final output will be a statically linked executable. *TODO*
...
### Runtime binding.
Doe uses `libtcc` to JIT compile the bindings from `extern` declarations.
An example can be found in [ffi.cy](https://github.com/azhai/doe/blob/master/examples/ffi.cy).
...
`libdoe` allows embedding the Doe compiler and VM into an application. Doe's builtin types, functions, and the CLI app were built using `libdoe`.

The API is defined in the [C header file](https://github.com/azhai/doe/blob/master/src/include/cyber.h).
The examples shown below can be found in the repository under [libdoe](https://github.com/azhai/doe/blob/master/examples/libdoe). The examples are in C, but it can be easily translated to C++ or any C-ABI compatible language.

Types and constants from the C-API begin with `CL` and functions begin with `cl`.

## Create VM.
A VM instance is required to compile and interpret Doe code. To create a new VM instance, call `cl_vm_init`:
```c
#include "cyber.h"
...
A VM function is a C function that the VM can call into. Doe is a statically typed language so it requires the return and parameters to be defined by how much space they occupy on the stack:
```c
...
Unlike conventional Doe code, the REPL allows variable redeclarations:
```bash
> a := 'a is now a string'
> a
